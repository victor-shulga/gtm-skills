...
When a scoring category has no/empty/unverified data, **cap that category at 50% of its max** — do not
default it to the midpoint. A blank field is unknown, not "fine".
- no LinkedIn / engagement data → engagement category capped at 50%
- no live signal detected → signal category capped at 50%
- no firmographic match (size/industry unknown) → ICP-fit capped at 50%

Always state in the account's scoring reasoning which categories were capped — so a low score from
thin data is never confused with a low score from a real bad-fit.
